...
export default function IDE({ project, onUpdateProject, onDeleteProject, onBack }: IDEProps) {
  ...
  return (
    <div className="flex h-screen bg-[#f3f3f3] overflow-hidden text-[#1a1a1a] font-sans selection:bg-gray-200">
      ...
      <div className="hidden md:flex w-12 bg-[#1a1a1a] flex-col items-center py-4 gap-4 shrink-0 z-50">
        <button onClick={onBack} className="p-2 text-gray-500 hover:text-white transition-colors">
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1 w-full flex flex-col items-center gap-4">
          <button 
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className={`p-2 transition-colors ${isSidebarOpen ? 'text-white' : 'text-gray-600 hover:text-white'}`}
            title="Explorer"
          >
            <File size={20} />
          </button>
          <button 
            onClick={handleBuild}
            title="Build APK"
            className={`p-2 transition-colors ${buildStatus.status !== 'idle' ? 'text-gray-300' : 'text-gray-600 hover:text-white'}`}
          >
            <CloudUpload size={20} />
          </button>
          <button 
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 flex overflow-hidden relative">
              ...
              {(isSidebarOpen && (!activeFileId || window.innerWidth >= 768)) && (
                <aside className="w-full md:w-64 flex flex-col vs-sidebar shrink-0 shadow-lg z-40 border-r border-gray-200 bg-white">
                  <div className="vs-header justify-between shrink-0 bg-[#e8e8e8]">
                    <span className="text-[9px] font-black tracking-[0.2em]">Workspace</span>
              ...
              {(activeFileId || window.innerWidth >= 768) && (
                <main className={`flex-1 flex flex-col min-w-0 bg-[#ffffff] relative z-30 transition-all ${
                  activeFileId && window.innerWidth < 768 ? 'fixed inset-0 top-[96px] z-50' : ''
                }`}>
                  {activeFile ? (
                    ...
                    <div className="hidden md:flex flex-1 flex-col items-center justify-center p-12 bg-[#ffffff]">
                      ...
                      <h2 className="text-2xl font-black text-[#1a1a1a] uppercase italic tracking-tighter">APP STUDIO IDE</h2>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/10 backdrop-blur-[1px]">
          <div className="bg-white border border-gray-200 rounded-xl shadow-2xl w-full max-w-md overflow-hidden ring-1 ring-black/5">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-100">
                {isDeleting ? (
                  ...
                  <button
                    onClick={() => setIsDeleting(true)}
                    className="px-4 py-2.5 text-[10px] font-black text-red-500 hover:bg-red-50 rounded-lg transition-colors uppercase tracking-widest"
                  >
                    DELETE PROJECT
                  </button>
                )}
                ...
                <button
                  ...
                  className="px-4 py-2.5 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-lg font-bold text-[10px] transition-all"
                >
                  CANCEL
                </button>
                <button
                  ...
                  className="px-4 py-2.5 bg-black text-white rounded-lg font-bold text-[10px] shadow-sm transition-all"
                >
                  SAVE & SYNC
                </button>
              </div>
